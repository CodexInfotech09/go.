...
# Dictionary to store pending attack requests
pending_attacks = {}
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if user_id not in admin_id:
            if user_id in attack_cooldown and (datetime.datetime.now() - attack_cooldown[user_id]).seconds < 60:
                response = "You are on cooldown. Please wait 1 minute before running the /attack command again."
                bot.reply_to(message, response)
                return
            attack_cooldown[user_id] = datetime.datetime.now()

        # Store the user ID in pending_attacks to track that they need to provide further information
        pending_attacks[user_id] = True
        bot.reply_to(message, "Please provide the target IP, port, and time in the following format:\n<IP> <port> <time>")
    else:
...
# Handler for follow-up message with IP, port, and time
    ...
    if len(command) != 3:
        ...
        try:
            ...
            if time > 600:
                ...
                # Remove the user from pending_attacks as the process is completed
                del pending_attacks[user_id]
        except ValueError:
